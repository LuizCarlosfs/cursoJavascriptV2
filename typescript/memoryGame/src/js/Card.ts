...
function sortearNumero(min: number, max: number) {
...
while (cards.length < len) {
    let indiceExcluir = sortearNumero(0, cardsOriginais.length - 1)
    const exclusao = cardsOriginais.splice(indiceExcluir, 1)
